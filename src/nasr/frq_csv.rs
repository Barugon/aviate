...
impl Source {
  ...
  /// - `id`: airport ID
  /// - `cancel`: cancellation object
  pub fn frequencies(&self, id: &util::StackString, cancel: &util::Cancel) -> Vec<Frequency> {
    use vector::LayerAccess;
...
impl Frequency {
  fn new(feature: vector::Feature, fields: &Fields) -> Option<Self> {
    Some(Self {
      freq: common::get_field_as_str(&feature, fields.freq)?.into(),
      freq_use: common::get_field_as_str(&feature, fields.freq_use)?.into(),
      facility_type: common::get_field_as_str(&feature, fields.facility_type)?.into(),
      sectorization: common::get_field_as_str(&feature, fields.sectorization)?.into(),
      tower_call: common::get_field_as_str(&feature, fields.tower_or_comm_call)?.into(),
      approach_call: common::get_field_as_str(&feature, fields.primary_approach_radio_call)?.into(),
      remark: common::get_field_as_str(&feature, fields.remark)?.into(),
    })
  }
