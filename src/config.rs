...
impl Storage {
  pub fn new() -> Self {
    ...
    let bounds = Json::parse_string(include_str!("../res/bounds.json"));
    Self { items, bounds }
  }

  pub fn get_chart_bounds(&self, chart_name: &str, chart_size: geom::Size) -> Vec<geom::Px> {
    fn get_defined_bounds(bounds: &Variant, chart_name: &str, limit: geom::Coord) -> Option<Vec<geom::Px>> {
      // Find the chart.
      let dictionary = ok!(bounds.try_to::<Dictionary>())?;
      let array = ok!(dictionary.get(chart_name)?.try_to::<Array<Variant>>())?;
    ...
    if let Some(points) = get_defined_bounds(&self.bounds, chart_name, limit) {
      points
    } else {
...
/// > SVGs are generated by tracing chart boundaries (clockwise) using the path tool in GIMP.
#[cfg(feature = "dev")]
